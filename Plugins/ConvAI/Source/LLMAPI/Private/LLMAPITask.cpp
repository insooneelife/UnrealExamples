...
void FLLMAPITask::ProcessLM()
{
	...
	TSharedPtr<FLLMAPITask> ThisShared = AsShared();
	TWeakPtr<FLLMAPITask> WeakThisPtr = ThisShared;

	// Set response callback
	Request->OnProcessRequestComplete().BindLambda([WeakThisPtr](FHttpRequestPtr InRequest, FHttpResponsePtr InResponse, bool bWasSuccessful)
		{
			SCOPE_CYCLE_COUNTER(STAT_ApiLM_Background);

			TSharedPtr<FLLMAPITask> This = WeakThisPtr.Pin();
			if (!This.IsValid())
			{
				UE_LOG(LogTemp, Error, TEXT("ProcessLM  Background Lambda, ThisPtr already deleted."));
				return;
			}
